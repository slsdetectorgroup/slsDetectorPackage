...
class PlotPattern():
    def __init__(self, pattern, colors_plot, colors_wait, linestyles_wait, alpha_wait, alpha_wait_rect, colors_loop, linestyles_loop, alpha_loop, alpha_loop_rect, clock_vertical_lines_spacing, show_clocks_number):
    ...
    def patternPlot(self):
        ...
        for idx, i in enumerate(range(nbiteff)):
            ...
            axs2[idx].plot(subMat.T[i], "-", drawstyle="steps-post", linewidth=2.0, color=self.colors_plot[idx % 2])
            ...
            axs2[idx].plot(x_additional, additional_stuff,
                        "--", drawstyle="steps-post", linewidth=2.0, color=self.colors_plot[idx % 2], alpha=0.5)
            axs2[idx].yaxis.set_ticks([0.5], minor=False)
            axs2[idx].xaxis.set_ticks(arange(0, len(subMat.T[i]) + 10, self.clock_vertical_lines_spacing))

            axs2[idx].yaxis.set_ticklabels([avail_name[i]])
            axs2[idx].get_yticklabels()[0].set_color(self.colors_plot[idx % 2])

            axs2[idx].grid(1, 'both', 'both', alpha=0.5)
            axs2[idx].yaxis.grid(which="both", color=self.colors_plot[idx % 2], alpha=0.2)
            if idx != nbiteff - 1:
            ...
            if waittime0 is not None:
                if waittime0 == 0:
                    axs2[idx].plot([wait0, wait0], [-10, 10],
                                linestyle=self.linestyles_wait[0], color=self.colors_wait[0], alpha=self.alpha_wait[0], linewidth=2.0)
                    axs2[idx].plot([wait0 + 1, wait0 + 1], [-10, 10],
                                linestyle=self.linestyles_wait[0], color=self.colors_wait[0], linewidth=2.0, alpha=self.alpha_wait[0])
                    ...
                    axs2[idx].plot([wait0, wait0], [-10, 10],
                                ...
                                linewidth=2.0, alpha=self.alpha_wait[0])

            # Wait 1
            if waittime1 is not None:
                if waittime1 == 0:
                    axs2[idx].plot([wait1, wait1], [-10, 10],
                                linestyle=self.linestyles_wait[1], color=self.colors_wait[1], alpha=self.alpha_wait[1], linewidth=2.0)
                    axs2[idx].plot([wait1 + 1, wait1 + 1], [-10, 10],
                                linestyle=self.linestyles_wait[1], color=self.colors_wait[1], linewidth=2.0, alpha=self.alpha_wait[1])
                    ...
                    axs2[idx].plot([wait1, wait1], [-10, 10],
                                ...
                                linewidth=2.0, alpha=self.alpha_wait[1])

            # Wait 2
            if waittime2 is not None:
                if waittime2 == 0:
                    axs2[idx].plot([wait2, wait2], [-10, 10],
                                linestyle=self.linestyles_wait[2], color=self.colors_wait[2], alpha=self.alpha_wait[2], linewidth=2.0)
                    axs2[idx].plot([wait2 + 1, wait2 + 1], [-10, 10],
                                linestyle=self.linestyles_wait[2], color=self.colors_wait[2], linewidth=2.0, alpha=self.alpha_wait[2])
                    ...
                    axs2[idx].plot([wait2, wait2], [-10, 10],
                                ...
                                linewidth=2.0, alpha=self.alpha_wait[2])

            # Wait 3
            if waittime3 is not None:
                if waittime3 == 0:
                    axs2[idx].plot([wait3, wait3], [-10, 10],
                                linestyle=self.linestyles_wait[3], color=self.colors_wait[3], alpha=self.alpha_wait[3], linewidth=2.0)
                    axs2[idx].plot([wait3 + 1, wait3 + 1], [-10, 10],
                                linestyle=self.linestyles_wait[3], color=self.colors_wait[3], linewidth=2.0, alpha=self.alpha_wait[3])
                    ...
                    axs2[idx].plot([wait3, wait3], [-10, 10],
                                ...
                                linewidth=2.0, alpha=self.alpha_wait[3])

            # Wait 4
            if waittime4 is not None:
                if waittime4 == 0:
                    axs2[idx].plot([wait4, wait4], [-10, 10],
                                linestyle=self.linestyles_wait[4], color=self.colors_wait[4], alpha=self.alpha_wait[4], linewidth=2.0)
                    axs2[idx].plot([wait4 + 1, wait4 + 1], [-10, 10],
                                linestyle=self.linestyles_wait[4], color=self.colors_wait[4], linewidth=2.0, alpha=self.alpha_wait[4])
                    ...
                    axs2[idx].plot([wait4, wait4], [-10, 10],
                                ...
                                linewidth=2.0, alpha=self.alpha_wait[4])

            # Wait 5
            if waittime5 is not None:
                if waittime5 == 0:
                    axs2[idx].plot([wait5, wait5], [-10, 10],
                                linestyle=self.linestyles_wait[5], color=self.colors_wait[5], alpha=self.alpha_wait[5], linewidth=2.0)
                    axs2[idx].plot([wait5 + 1, wait5 + 1], [-10, 10],
                                linestyle=self.linestyles_wait[5], color=self.colors_wait[5], linewidth=2.0, alpha=self.alpha_wait[5])
                    ...
                    axs2[idx].plot([wait5, wait5], [-10, 10],
                                ...
                                linewidth=2.0, alpha=self.alpha_wait[5])
            ...
            if nloop0 is not None:
                if nloop0 == 0:
                    axs2[idx].plot([loop0_start, loop0_start], [-10, 10],
                                ...
                                alpha=self.alpha_loop[0], linewidth=2.0)
                    axs2[idx].plot([loop0_end + 1, loop0_end + 1], [-10, 10],
                                linestyle=self.linestyles_loop[0], color=self.colors_loop[0], alpha=self.alpha_loop[0], linewidth=2.0)
            ...
            if nloop1 is not None:
                if nloop1 == 0:
                    axs2[idx].plot([loop1_start, loop1_start], [-10, 10],
                                ...
                                alpha=self.alpha_loop[1], linewidth=2.0)
                    axs2[idx].plot([loop1_end + 1, loop1_end + 1], [-10, 10],
                                linestyle=self.linestyles_loop[1], color=self.colors_loop[1], alpha=self.alpha_loop[1], linewidth=2.0)
            ...
            if nloop2 is not None:
                if nloop2 == 0:
                    axs2[idx].plot([loop2_start, loop2_start], [-10, 10],
                                ...
                                alpha=self.alpha_loop[2], linewidth=2.0)
                    axs2[idx].plot([loop2_end + 1, loop2_end + 1], [-10, 10],
                                linestyle=self.linestyles_loop[2], color=self.colors_loop[2], alpha=self.alpha_loop[2], linewidth=2.0)
            ...
            if nloop3 is not None:
                if nloop3 == 0:
                    axs2[idx].plot([loop3_start, loop3_start], [-10, 10],
                                ...
                                alpha=self.alpha_loop[3], linewidth=2.0)
                    axs2[idx].plot([loop3_end + 1, loop3_end + 1], [-10, 10],
                                linestyle=self.linestyles_loop[3], color=self.colors_loop[3], alpha=self.alpha_loop[3], linewidth=2.0)
            ...
            if nloop4 is not None:
                if nloop4 == 0:
                    axs2[idx].plot([loop4_start, loop4_start], [-10, 10],
                                ...
                                alpha=self.alpha_loop[4], linewidth=2.0)
                    axs2[idx].plot([loop4_end + 1, loop4_end + 1], [-10, 10],
                                linestyle=self.linestyles_loop[4], color=self.colors_loop[4], alpha=self.alpha_loop[4], linewidth=2.0)
            ...
            if nloop5 is not None:
                if nloop5 == 0:
                    axs2[idx].plot([loop5_start, loop5_start], [-10, 10],
                                ...
                                alpha=self.alpha_loop[5], linewidth=2.0)
                    axs2[idx].plot([loop5_end + 1, loop5_end + 1], [-10, 10],
                                linestyle=self.linestyles_loop[5], color=self.colors_loop[5], alpha=self.alpha_loop[5], linewidth=2.0)
