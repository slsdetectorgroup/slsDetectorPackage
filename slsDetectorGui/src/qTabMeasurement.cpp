...
qTabMeasurement::qTabMeasurement(qDetectorMain *parent, multiSlsDetector *&detector, qDrawPlot *&plot) : thisParent(parent), myDet(detector), myPlot(plot), expertMode(false) {
    setupUi(this);
    SetupWidgetWindow();
    Initialization();
    SetupTimingMode();
...
void qTabMeasurement::GetModeFromDetector(bool startup) {
    ...
    // on which all modes are allowed in detectors
        ...
        qDefs::Message(qDefs::WARNING, "Unknown Timing Mode detected from detector."
                       ...
                       "qTabMeasurement::GetModeFromDetector");
        comboTimingMode->setCurrentIndex((int)None);
        SetTimingMode((int)None);
    }
...
void qTabMeasurement::setExposureTime() {
    ...
    double exptimeNS = qDefs::getNSTime((qDefs::timeUnit)comboExpUnit->currentIndex(), spinExpTime->value());
#ifdef VERBOSE
    cout << "Setting acquisition time to " << exptimeNS << " clocks"
         << "/" << spinExpTime->value() << qDefs::getUnitString((qDefs::timeUnit)comboExpUnit->currentIndex()) << endl;
#endif
...
void qTabMeasurement::setAcquisitionPeriod() {
    ...
    double acqtimeNS = qDefs::getNSTime((qDefs::timeUnit)comboPeriodUnit->currentIndex(), spinPeriod->value());
#ifdef VERBOSE
    cout << "Setting frame period between exposures to " << acqtimeNS << " clocks"
         << "/" << spinPeriod->value() << qDefs::getUnitString((qDefs::timeUnit)comboPeriodUnit->currentIndex()) << endl;
#endif
...
#ifdef VERBOSE
    cout << "Setting delay after trigger to " << exptimeNS << " clocks"
         << "/" << spinDelay->value() << qDefs::getUnitString((qDefs::timeUnit)comboDelayUnit->currentIndex()) << endl;
#endif
...
#ifdef VERBOSE
    cout << "**Updated Measurement Tab" << endl
         << endl;
#endif
}
...
void qTabMeasurement::EnableProbes() {
    ...
    if ((spinNumProbes->isEnabled()) && (spinNumProbes->value() > 0) && (spinNumTriggers->value() != 1)) {
        qDefs::Message(qDefs::INFORMATION, "<nobr>Number of Triggers has been reset to 1.</nobr><br>"
                                           "<nobr>This is mandatory to use probes.</nobr>",
                       "qTabMeasurement::EnableProbes");
