...
void qTabSettings::GetSettings() {
	int sett = static_cast<int>(myDet->getSettings());
...
void qTabSettings::GetDynamicRange() {
	...
	qDefs::checkErrorMessage(myDet, "qTabSettings::GetDynamicRange");
	...
	case 4:
		...
		break;
	default:
		qDefs::Message(qDefs::WARNING, "Unknown Dyanmic Range " + std::to_string(ret) + "." , "qTabSettings::SetupDetectorSettings");
		break;
	}
}
...
void qTabSettings::SetDynamicRange(int index) {
	...
	//check
	if (ret != dr) {
		qDefs::Message(qDefs::WARNING, "Could not set dynamic range.", "qTabSettings::SetDynamicRange");
...
void qTabSettings::SetEnergy() {
	...
	if ((ret - index) > 200) {
		qDefs::Message(qDefs::WARNING, "Threshold energy could not be set (tolerance 200).", "qTabSettings::SetEnergy");
	}
