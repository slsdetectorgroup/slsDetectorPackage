...
#define CMD_ARM_LOAD_BIT_FILE "~/fpgautil/fpgautil -b /root/apps/xilinx-ctb/XilinxCTB.bit -f Full"
#define CMD_ARM_DEVICE_TREE_API_FOLDER "/sys/kernel/config/device-tree/overlays/spidr"
...
int createDeviceTree(char *mess) {
    char cmd[MAX_STR_LENGTH] = {0};
    memset(cmd, 0, MAX_STR_LENGTH);
    sprintf(cmd, CMD_ARM_LOAD_DEVICE_TREE_FORMAT, CMD_ARM_DEVICE_TREE_OVERLAY_FILE, CMD_ARM_DEVICE_TREE_API_FOLDER);
    char retvals[MAX_STR_LENGTH] = {0};
    memset(retvals, 0, MAX_STR_LENGTH);
    if (executeCommand(cmd, retvals, logDEBUG1) == FAIL) {
        snprintf(mess, MAX_STR_LENGTH,
...
int verifyDeviceTree(char* mess) {
    ...
    for (int i = 0; i != 5; ++i) {
        ...
        if (strstr(retvals, deviceNames[i]) == NULL) {
            snprintf(mess, MAX_STR_LENGTH,
                     "Could not verify device tree. Device %s expected %s but got %s\n",
