...
int getDacRegister(int dacnum) {
  ...
  printf("READ dac register value %d address %d\n",(int16_t)bus_r(DAC_VAL_REG),bus_r(DAC_NUM_REG)) ;
  return (int16_t)bus_r(DAC_VAL_REG);  
}
...
double setDAC(enum DACINDEX ind, double val, int imod, int mV){
	...
	//select dac-chip:
	if  (myDetectorType==JUNGFRAUCTB || myDetectorType==MYTHEN3){
    	ichip=2-ind/8;
    	printf("This is a CTB (or mythen3)\n");
    }
    else {
    	ichip=ind/8;
...
int setPower(int ind, int val) {
  ...
  if (val==-1) {
    ...
    if (pwrindex>=0) { // v_a, v_b, v_c, v_d, v_io
      printf("vpower\n");
  ...
  if (pwrindex>=0 && val!=-1) {// v_a, v_b, v_c, v_d, v_io
    preg=bus_r(POWER_ON_REG);
    printf("power reg is %08x\n",bus_r(POWER_ON_REG));
    printf("Switching off power %d\n", pwrindex);
    bus_w(POWER_ON_REG,preg&(~(1<<(16+pwrindex))));
    setDAC(dacindex,-100,-1,1); 
    printf("power reg is %08x\n",bus_r(POWER_ON_REG));
    retval=0;
  }
  
  if (dacindex>0 && dacval!=-100) {// v_a, v_b, v_c, v_d, v_io, v_chip
    
    printf("Setting power %d to %d mV\n",ind,val);
    printf("Setting DAC %d to value %d\n",dacindex,dacval);
    retval=setDAC(dacindex,dacval,-1,1);// in mV
    if (pwrindex>=0 && dacval>=0 ) {// v_a, v_b, v_c, v_d, v_io
      preg=bus_r(POWER_ON_REG);
      printf("power reg is %08x\n",bus_r(POWER_ON_REG));
  ...
  if (pwrindex>=0) {// v_a, v_b, v_c, v_d, v_io
    if (bus_r(POWER_ON_REG)&(1<<(16+pwrindex))){
      vmax=2700-(getDacRegister(19)*1000)/4095-200; // 19=v_chip
      printf("Vchip id %d mV\n",vmax+200);
      // retval1=vmax-(retval*(vmax-vmin))/4095;
      retval1=vmax-retval; // mV
      printf("Vdac id %d mV\n",retval1);
	if (retval1>vmax)
	  retval1=vmax;
	if (retval1<vmin)
