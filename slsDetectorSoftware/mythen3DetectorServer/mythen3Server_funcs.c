...
#include "mythen3FunctionList.h"
#include "communication_funcs.h"
#include "mythen3Server_defs.h"
// #include "mythen3RegisterDefs.h"
// #include "blackfin.h"
...
#endif
*/
enum detectorType myDetectorType=MYTHEN3;
...
// #endif
	...
	lockStatus=0;
	#ifdef MYTHEN3D
printf("This is a mythen3 detector\n");
	#endif
}
	...
	switch (ind) {
		...
		default:
			if(ind<0){
				...
				printf("The dac has no name yet. \n");
			}
			break;
	}
	...
	if (ret==OK) {
		if (differentClients && lockStatus && val!=-1) {
			...
			if(ind == V_POWER_A || ind == V_POWER_B || ind == V_POWER_IO){
				printf("Setting a power: %d \n",ind);
				if(mV){

					if( val >=0 && val <=vPowerLimit){
						retval[1] = setPower(ind,val); // in mV
						retval[0] = retval[1]*4095/2500.;
					}
					else if(val==-1){
						retval[1] = setPower(ind,val);
						retval[0] = retval[1]*4095/2500.;
						printf("READ ONLY: val=-1, read %f mV",retval[1]);
					}
					else{
					...
					printf("********power %d should be set in mV instead of DACu", ind);
				}
			}
			else if(ind == V_LIMIT){
				if(mV){ // without safetycheck
					retval[1] = setPower(ind, val);
					retval[0] = 0; // to avoid wrong error messages
				}
				else{
					printf("********power %d should be set in mV instead of DACu", ind);
				}
			...
			//dac:
				...
				if(val==-1){
					...
					retval[1]=retval[0]*2500./4095;
					printf("READ ONLY: val=-1, read %4.0f DAC \n",retval[0]);
				}
				else if(mV && (val>0 && val<2500)){ // val given in mV, check that val is not out of range
					int dacval = (int)val*4095./2500.; // convert to DACu
					retval[1] = setDAC(idac,dacval,imod,mV);
				...
				if(retval[0]==-1 || retval[1] == -1){// something in setDAC failed (should never occur)
					ret = FAIL;
					sprintf(mess,"Setting DAC %d of module %d: wrote %d but read %f\n", idac, imod, val, temp);
					cprintf(RED, "Warning: %s", mess);
				}
				if(mV)
					temp = retval[1];
...
//#ifdef VERBOSE
	if(ret==OK){
		printf("DAC set to %4.0f in dac units and %f mV\n",  retval[0],retval[1]);
	}
	else{
		printf("ERROR! Not able to set DAC to %4.0f in dac units and %f mV\n",  retval[0],retval[1]);
	}
//#endif
