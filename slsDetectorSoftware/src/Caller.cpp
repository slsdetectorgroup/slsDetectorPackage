...
os << R"V0G0N([0, 1] 
	[Eiger] 1 is default. 0 deactivates readout and does not send data. )V0G0N" << std::endl;
return os.str();
}
...
throw RuntimeError("INTERNAL ERROR: Invalid action: supported actions are ['GET', 'PUT']");
}
...
os << R"V0G0N([n_clk in MHz]
	[Ctb] ADC clock frequency in MHz. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([bitmask]
	[Ctb] ADC Enable Mask for 1Gb Enable for each 32 ADC channel. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([bitmask]
	[Ctb] ADC Enable Mask for 10Gb mode for each 32 ADC channel. However, if any of a consecutive 4 bits are enabled, the complete 4 bits are enabled. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([name] 
		[Ctb][Xilinx_Ctb] Get the adc index for the given name. )V0G0N" << std::endl;
return os.str();
}
...
throw RuntimeError("INTERNAL ERROR: Invalid action: supported actions are ['GET']");
}

// generate code for each action
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 1) {
if (det->getDetectorType().squash(defs::GENERIC) != defs::CHIPTESTBOARD && det->getDetectorType().squash(defs::GENERIC) != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute adcindex at module level"); } 
...
os << R"V0G0N([bitmask]
	...
	[Jungfrau][Moench] Inversions on top of the default mask. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([adcname1 adcname2 .. adcname32] 
		[Ctb][Xilinx_Ctb] Set the list of adc names for this board. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 0) {
auto det_type = det->getDetectorType().squash(defs::GENERIC);;
if (cmd != "daclist" && det_type != defs::CHIPTESTBOARD && det_type != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute adclist at module level"); } 
...
auto det_type = det->getDetectorType().squash(defs::GENERIC);;
if (cmd != "daclist" && det_type != defs::CHIPTESTBOARD && det_type != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (cmd == "daclist" && det_type != defs::CHIPTESTBOARD && det_type != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError("This detector already has fixed dac names. Cannot change them."); }
if (det_id != -1) { throw RuntimeError("Cannot execute adclist at module level"); } 
...
os << R"V0G0N([0-31][name] 
		[Ctb][Xilinx_Ctb] Set the adc at the given position to the given name. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 1) {
if (det->getDetectorType().squash(defs::GENERIC) != defs::CHIPTESTBOARD && det->getDetectorType().squash(defs::GENERIC) != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute adcname at module level"); } 
auto arg0 = StringTo<int>(args[0]);
...
if (args.size() == 0) {
auto det_type = det->getDetectorType().squash(defs::GENERIC);;
if (det_type == defs::EIGER || det_type == defs::MYTHEN3 || det_type == defs::GOTTHARD2) { throw RuntimeError("adcphase not implemented for this detector"); }
...
auto det_type = det->getDetectorType().squash(defs::GENERIC);;
if (det_type == defs::EIGER || det_type == defs::MYTHEN3 || det_type == defs::GOTTHARD2) { throw RuntimeError("adcphase not implemented for this detector"); }
if (args[0] != "deg") { throw RuntimeError("Unknown adcphase   argument " + args[0] + ". Did you mean deg?    "); }
...
auto det_type = det->getDetectorType().squash(defs::GENERIC);;
if (det_type == defs::EIGER || det_type == defs::MYTHEN3 || det_type == defs::GOTTHARD2) { throw RuntimeError("adcphase not implemented for this detector"); }
if (args[1] != "deg") { throw RuntimeError("Unknown adcphase 2nd argument " + args[1] + ". Did you mean deg?"); }
auto arg0 = StringTo<int>(args[0]);
...
os << R"V0G0N([n_value]
	[Ctb][Moench] Pipeline for ADC clock. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([address] [value]
	[Jungfrau][Moench][Ctb][Gotthard] Writes to an adc register in hex. Advanced user Function! )V0G0N" << std::endl;
return os.str();
}
...
throw RuntimeError("INTERNAL ERROR: Invalid action: supported actions are ['PUT']");
}
...
if (args.size() == 1) {
if ((args[0] != "mv") && (args[0] != "mV")) { throw RuntimeError("Unknown argument " + args[0] + ". Did you mean mV?"); }
...
if (args.size() == 2) {
if ((args[1] != "mv") && (args[1] != "mV")) { throw RuntimeError("Unknown argument " + args[1] + ". Did you mean mV?"); }
auto arg0 = StringTo<int>(args[0]);
...
os << R"V0G0N([0, 1]
	[Mythen3] Enables or disables analog pulsing. Default is disabled )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_samples]
	[Ctb] Number of analog samples expected. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	...
	Default is 0 or this mode disabled(comparator enabled throughout). 1 enables mode. 0 disables mode.  )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	[Eiger][Jungfrau][Moench] Sends software trigger signal to detector and blocks till the frames are sent out for that trigger. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 0) {
bool block = true;
if (det_id != -1) { throw RuntimeError("Cannot execute blockingtrigger at module level"); } 
...
os << R"V0G0N([duration] [(optional unit) ns|us|ms|s]
	[Gotthard2] Period between 2 bursts. Only in burst mode and auto timing mode. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_bursts]
	[Gotthard2] Number of bursts per aquire. Only in auto timing mode and burst mode. Use timing command to set timing mode and burstmode command to set burst mode. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (det_id != -1) { throw RuntimeError("Cannot execute bursts at module level"); } 
...
os << R"V0G0N(
	[Gotthard2] Number of bursts left in acquisition. Only in burst auto mode. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	[Gotthard2] Enable or disable CDS gain. Default is disabled. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 2) {
if (StringTo<int>(args[1]) < 0 || StringTo<int>(args[1]) > 31) { throw RuntimeError("Bit number out of range: " + args[1]); }
auto arg0 = StringTo<uint32_t>(args[0]);
...
os << R"V0G0N(
	If acquisition aborted during acquire command, use this to clear acquiring flag in shared memory before starting next acquisition )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 0) {
if (det_id != -1) { throw RuntimeError("Cannot execute clearbusy at module level"); } 
...
os << R"V0G0N([Gotthard] Resets Region of interest in detector. All channels enabled. Default is all channels enabled. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_clock (0-5)] [n_divider]
	[Gotthard2][Mythen3] Clock Divider of clock n_clock. Must be greater than 1. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 1) {
defs::detectorType type = det->getDetectorType().squash(defs::GENERIC);;
if (type != defs::GOTTHARD2 && type != defs::MYTHEN3) { throw RuntimeError("clkdiv not implemented for this detector."); }
auto arg0 = StringTo<int>(args[0]);
...
os << R"V0G0N([n_clock (0-5)] [freq_in_Hz]
	[Gotthard2][Mythen3] Frequency of clock n_clock in Hz. Use clkdiv to set frequency. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 1) {
defs::detectorType type = det->getDetectorType().squash(defs::GENERIC);;
if (type != defs::GOTTHARD2 && type != defs::MYTHEN3) { throw RuntimeError("clkfreq not implemented for this detector."); }
auto arg0 = StringTo<int>(args[0]);
...
os << R"V0G0N([n_clock (0-5)] [phase] [deg (optional)]
	[Gotthard2][Mythen3] Phase of clock n_clock. If deg, then phase shift in degrees, else absolute phase shift values. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 1) {
defs::detectorType type = det->getDetectorType().squash(defs::GENERIC);;
if (type != defs::GOTTHARD2 && type != defs::MYTHEN3) { throw RuntimeError("clkphase not implemented for this detector."); }
auto arg0 = StringTo<int>(args[0]);
...
defs::detectorType type = det->getDetectorType().squash(defs::GENERIC);;
if (type != defs::GOTTHARD2 && type != defs::MYTHEN3) { throw RuntimeError("clkphase not implemented for this detector."); }
if (args[1] != "deg") { throw RuntimeError("Cannot scan argument" + args[1] + ". Did you mean deg?"); }
auto arg0 = StringTo<int>(args[0]);
...
defs::detectorType type = det->getDetectorType().squash(defs::GENERIC);;
if (type != defs::GOTTHARD2 && type != defs::MYTHEN3) { throw RuntimeError("clkphase not implemented for this detector."); }
if (args[2] != "deg") { throw RuntimeError("Cannot scan argument" + args[2] + ". Did you mean deg?"); }
auto arg0 = StringTo<int>(args[0]);
auto arg1 = StringTo<int>(args[1]);
...
os << R"V0G0N([value]
	...
	Gui uses it to rearrange for complete image )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([duration] [(optional unit) ns|us|ms|s]
	[Jungfrau] Time before end of exposure when comparator is disabled. It is only possible for chipv1.1. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([chip index 0-9, -1 for all] [adc index 0-31, -1 for all] [7 bit configuration value in hex]
	[Gotthard2] Sets  configuration for specific chip and adc, but configures 1 chip (all adcs for that chip) at a time. )V0G0N" << std::endl;
return os.str();
}
...
os << '['<< args[0]<< ", "<< args[1]<< ", "<< ToStringHex( StringTo<int>(args[2]))<< "]" << '\n';
}
...
os << R"V0G0N(
	Frees shared memory before loading configuration file. Set up once. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (det_id != -1) { throw RuntimeError("Cannot execute config at module level"); } 
...
os << R"V0G0N(
	[Xilinx Ctb] Waits for transceiver to be aligned. Chip had to be configured (powered on) before this. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 1) {
defs::dacIndex dacIndex = ((det->getDetectorType().squash() == defs::CHIPTESTBOARD || det->getDetectorType().squash() == defs::XILINX_CHIPTESTBOARD) && !is_int(args[0])) ? det->getDacIndex(args[0]) : StringTo<defs::dacIndex>(args[0]);
try {
StringTo<bool>("0");
} catch (...) {
  throw RuntimeError("Could not convert argument 1 to bool");
...
if (args.size() == 1) {
defs::dacIndex dacIndex = ((det->getDetectorType().squash() == defs::CHIPTESTBOARD || det->getDetectorType().squash() == defs::XILINX_CHIPTESTBOARD) && !is_int(args[0])) ? det->getDacIndex(args[0]) : StringTo<defs::dacIndex>(args[0]);
if (is_int(args[0]) && det->getDetectorType().squash() != defs::CHIPTESTBOARD && det->getDetectorType().squash() != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError("Dac indices can only be used for chip test board. Use daclist to get list of dac names for current detector."); }
auto arg1 = StringTo<bool>("0");
...
os << R"V0G0N([name] 
		[Ctb][Xilinx_Ctb] Get the dac index for the given name. )V0G0N" << std::endl;
return os.str();
}
...
defs::dacIndex index = defs::DAC_0;
if (det->getDetectorType().squash(defs::GENERIC) != defs::CHIPTESTBOARD && det->getDetectorType().squash(defs::GENERIC) != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute dacindex at module level"); } 
...
os << R"V0G0N([dacname1 dacname2 .. dacname18] 
		...
		[All] Gets the list of dac names for every dac for this detector. )V0G0N" << std::endl;
return os.str();
}
...
auto det_type = det->getDetectorType().squash(defs::GENERIC);;
if (cmd != "daclist" && det_type != defs::CHIPTESTBOARD && det_type != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute daclist at module level"); } 
...
os << R"V0G0N([0-17][name] 
		[Ctb][Xilinx_Ctb] Set the dac at the given position to the given name. )V0G0N" << std::endl;
return os.str();
}
...
defs::dacIndex index = defs::DAC_0;
if (det->getDetectorType().squash(defs::GENERIC) != defs::CHIPTESTBOARD && det->getDetectorType().squash(defs::GENERIC) != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute dacname at module level"); } 
auto t = det->getDacName(static_cast<defs::dacIndex>(StringTo<int>(args[0]) + index));
os << args[0]<< ' '<< t << '\n';
}
...
if (det->getDetectorType().squash(defs::GENERIC) != defs::CHIPTESTBOARD && det->getDetectorType().squash(defs::GENERIC) != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute dacname at module level"); } 
det->setDacName(static_cast<defs::dacIndex>(StringTo<int>(args[0]) + index), args[1]);
os << ToString(args) << '\n';
}
...
os << R"V0G0N([left|right] [0, 1]
	[Eiger] Enables or disables data streaming from left or/and right side of detector for 10 GbE mode. 1 (enabled) by default. )V0G0N" << std::endl;
return os.str();
}
...
StringTo<defs::portPosition>(args[0]);
  ...
  throw RuntimeError("Could not convert argument 0 to defs::portPosition");
}
}
...
os << R"V0G0N([n_clk in MHz]
	[Ctb] Clock for latching the digital bits in MHz. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value] [(optional)deg]
	...
	[Ctb]Changing dbitclk also resets dbitphase and sets to previous values. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 0) {
auto det_type = det->getDetectorType().squash(defs::GENERIC);;
if (det_type == defs::EIGER || det_type == defs::MYTHEN3 || det_type == defs::GOTTHARD2 || det_type == defs::MOENCH) { throw RuntimeError("dbitphase not implemented for this detector"); }
...
auto det_type = det->getDetectorType().squash(defs::GENERIC);;
if (det_type == defs::EIGER || det_type == defs::MYTHEN3 || det_type == defs::GOTTHARD2 || det_type == defs::MOENCH) { throw RuntimeError("dbitphase not implemented for this detector"); }
if (args[0] != "deg") { throw RuntimeError("Unknown dbitphase argument " + args[0] + ". Did you mean deg?  "); }
...
auto det_type = det->getDetectorType().squash(defs::GENERIC);;
if (det_type == defs::EIGER || det_type == defs::MYTHEN3 || det_type == defs::GOTTHARD2 || det_type == defs::MOENCH) { throw RuntimeError("dbitphase not implemented for this detector"); }
if (args[1] != "deg") { throw RuntimeError("Unknown dbitphase 2nd argument " + args[1] + ". Did you mean deg?  "); }
auto arg0 = StringTo<int>(args[0]);
...
os << R"V0G0N([n_value]
	...
	[Ctb] Options: 0-255 )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([dac name][value][(optional)setting]
	...
	[Jungfrau][Moench][Mythen3] When settings is provided, it sets the default value only for that setting )V0G0N" << std::endl;
return os.str();
}
...
StringTo<defs::detectorSettings>(args[1]);
  ...
  throw RuntimeError("Could not convert argument 1 to defs::detectorSettings");
}
}
...
StringTo<defs::detectorSettings>(args[2]);
  ...
  throw RuntimeError("Could not convert argument 2 to defs::detectorSettings");
}
}
...
os << R"V0G0N(
	[Mythen3] Loads and runs default pattern in pattern generator. It is to go back to initial settings. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([duration] [(optional unit) ns|us|ms|s]
	[Jungfrau][Moench][Gotthard][Mythen3][Gotthard2][Ctb][Moench][Xilinx Ctb] Delay after trigger )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([nx] [ny]
	...
	By default, it adds module in y dimension for 2d detectors and in x dimension for 1d detectors packet header. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 2) {
det->setDetectorSize(defs::xy(StringTo<int>(args[0]),StringTo<int>(args[1])));
os << ToString(args) << '\n';
}
...
os << R"V0G0N([0-775]
	[Ctb] Delay for diode. Delay is in ps and max of 775 ps. Resolution is 25 ps. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	[Mythen3] Enables or disables digital pulsing. Default is disabled )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([value]
	...
	[Jungfrau][Moench][Gotthard][Ctb][Mythen3][Gotthard2][Xilinx Ctb] 16 )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (det_id != -1) { throw RuntimeError("Cannot execute dr at module level"); } 
auto arg0 = StringTo<int>(args[0]);
...
os << R"V0G0N([n_value]
	[Ctb] Number of digital samples expected. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([duration] [(optional unit) ns|us|ms|s]
	...
	[Mythen3] Exposure time of all gate signals in auto and trigger mode (internal gating). To specify gate index, use exptime1, exptime2, exptime3. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value]
	[Mythen3] Exposure time of gate signal 1 in auto and trigger mode (internal gating). )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value]
	[Mythen3] Exposure time of gate signal 2 in auto and trigger mode (internal gating). )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value]
	[Mythen3] Exposure time of gate signal 3 in auto and trigger mode (internal gating). )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([(optional unit) ns|us|ms|s]
	[Gotthard] Exposure time left for current frame.  )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0-15]
	...
	The #images = #frames x #triggers x (#extrastoragecells + 1). )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getNumberOfAdditionalStorageCells(std::vector<int>{ det_id });
os << OutString(t) << '\n';
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (det_id != -1) { throw RuntimeError("Cannot execute extrastoragecells at module level"); } 
auto arg0 = StringTo<int>(args[0]);
...
os << R"V0G0N([0, 1]
	[Ctb] Enable for external sampling signal for digital data to signal by extsampling src command. For advanced users only. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0-63]
	[Ctb] Sampling source signal for digital data. For advanced users only. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_signal] [signal_type]
	...
	[Mythen3] [0-7] [trigger_in_rising_edge|trigger_in_falling_edge|inversion_on|inversion_off]
	 where 0 is master input trigger signal, 1-3 is master input gate signals, 4 is busy out signal and 5-7 is master output gate signals. )V0G0N" << std::endl;
return os.str();
}
...
StringTo<defs::externalSignalFlag>(args[1]);
  ...
  throw RuntimeError("Could not convert argument 1 to defs::externalSignalFlag");
}
}
...
os << R"V0G0N([binary|hdf5]
	File format of data file. For HDF5, package must be compiled with HDF5 flags. Default is binary. )V0G0N" << std::endl;
return os.str();
}
...
StringTo<defs::fileFormat>(args[0]);
  ...
  throw RuntimeError("Could not convert argument 0 to defs::fileFormat");
}
}
...
os << R"V0G0N([0-12]
	[Jungfrau] Set Filter Cell. Only for chipv1.1. Advanced user Command )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([value] [Gotthard2][Jungfrau] Set filter resistor. Increasing values for increasing resistance.
	...
	[Jungfrau] Options: [0|1]. Default is 1. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value]
	File or Acquisition index. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	[Jungfrau][Moench][Gotthard][Mythen3][Gotthard2][Ctb][Xilinx Ctb] Firmware test, ie. reads a read fixed pattern from a register. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Firmware version of detector in format [0xYYMMDD] or an increasing 2 digit number for Eiger. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	...
	[Jungfrau][Moench] flips rows in the detector itself. For bottom module and number of interfaces must be set to 2. slsReceiver and slsDetectorGui does not handle. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	[Eiger][Jungfrau][Moench] 10GbE Flow Control. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	Enable or disable receiver master file. Default is 1. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
if (det_id != -1) { throw RuntimeError("Cannot execute fmaster at module level"); } 
...
os << R"V0G0N([name]
	File name prefix for output data file. Default is run. File name: [file name prefix]_d[detector index]_f[sub file index]_[acquisition/file index].raw. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	Enable or disable file overwriting. Default is 1. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([path]
	...
	If path does not exist, it will try to create it. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_frames]
	...
	[Gotthard2] Burst mode has a maximum of 2720 frames. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (det_id != -1) { throw RuntimeError("Cannot execute frames at module level"); } 
auto arg0 = StringTo<int64_t>(args[0]);
...
os << R"V0G0N([(optional unit) ns|us|ms|s]
	...
	[Gotthard2] not in burst and auto mode. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	Enable or disable receiver file write. Default is 1. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([dynamicgain|forceswitchg1|forceswitchg2|fixg1|fixg2|fixg0]
	[Jungfrau] Gain mode.
	CAUTION: Do not use fixg0 without caution, you can damage the detector!!! )V0G0N" << std::endl;
return os.str();
}
...
StringTo<defs::gainMode>(args[0]);
  ...
  throw RuntimeError("Could not convert argument 0 to defs::gainMode");
}
}
...
os << R"V0G0N([0, 1]
	[Eiger][Jungfrau][Moench] Include Gap pixels in client data call back in Detecor api. Will not be in detector streaming, receiver file or streaming. Default is 0.  )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
if (det_id != -1) { throw RuntimeError("Cannot execute gappixels at module level"); } 
...
os << R"V0G0N([duration] [(optional unit) ns|us|ms|s]
	[Mythen3] Gate Delay of all gate signals in auto and trigger mode (internal gating). )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([duration] [(optional unit) ns|us|ms|s]
	[Mythen3] Gate Delay of gate signal 1 in auto and trigger mode (internal gating). )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([duration] [(optional unit) ns|us|ms|s]
	[Mythen3] Gate Delay of gate signal 2 in auto and trigger mode (internal gating). )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([duration] [(optional unit) ns|us|ms|s]
	[Mythen3] Gate Delay of gate signal 3 in auto and trigger mode (internal gating). )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_gates]
	[Mythen3] Number of external gates in gating or trigger_gating mode (external gating). )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	...
	[Eiger] Hardware version of front FPGA on detector. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value]
	...
	[Jungfrau][Moench][Ctb] [0|60-200] )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredCurrent(defs::I_POWER_A, std::vector<int>{ det_id });
os << OutString(t) << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredCurrent(defs::I_POWER_B, std::vector<int>{ det_id });
os << OutString(t) << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredCurrent(defs::I_POWER_C, std::vector<int>{ det_id });
os << OutString(t) << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredCurrent(defs::I_POWER_D, std::vector<int>{ det_id });
os << OutString(t) << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredCurrent(defs::I_POWER_IO, std::vector<int>{ det_id });
os << OutString(t) << '\n';
}
...
os << R"V0G0N([0, 1]
	...
	[Eiger][Jungfrau][Moench] Only for Virtual servers. If 0, each pixel intensity incremented by 1. If 1, all pixels almost saturated. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
if (det_id != -1) { throw RuntimeError("Cannot execute initialchecks at module level"); } 
...
os << R"V0G0N([offset] [increment]
	[Gotthard2] Inject channels with current source for calibration. Offset is starting channel that is injected, increment determines succeeding channels to be injected. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	[Mythen3] Enables or disables interpolation. Default is disabled.  Interpolation mode enables all counters and disables vth3. Disabling sets back counter mask and vth3. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	[Eiger] 1 interrupts last subframe at required exposure time. 0 will wait for last sub frame to finish exposing. 0 is default. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Get kernel version on the detector including time and date. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Client IP Address that last communicated with the detector. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	[Ctb] Switches on/off all LEDs. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	Lock detector to one IP, 1: locks. Default is unlocked )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	...
	[Gotthard][Gotthard2][Mythen3][Eiger][Jungfrau][Moench] Gets if the current (half) module is master. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	[Jungfrau][Moench][Ctb] Absolute maximum Phase shift of ADC clock. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_clock (0-5)]
	[Gotthard2][Mythen3] Absolute Maximum Phase shift of clock n_clock. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 1) {
defs::detectorType type = det->getDetectorType().squash(defs::GENERIC);;
if (type != defs::GOTTHARD2 && type != defs::MYTHEN3) { throw RuntimeError("maxclkphaseshift not implemented for this detector."); }
auto arg0 = StringTo<int>(args[0]);
...
os << R"V0G0N(
	[Ctb][Jungfrau] Absolute maximum Phase shift of of the clock to latch digital bits. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([(optional unit) ns|us|ms|s]
	[Eiger] Measured frame period between last frame and previous one. Can be measured with minimum 2 frames in an acquisition. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([(optional unit) ns|us|ms|s]
	[Eiger] Measured sub frame period between last sub frame and previous one. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	[Gotthard2][Eiger][Mythen3][Jungfrau][Moench] 16 bit value (ideally unique) that is streamed out in the UDP header of the detector. Picked up from a file on the module. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value]
	[Eiger][Jungfrau][Moench][Ctb][Xilinx Ctb] Next frame number. Stopping acquisition might result in different frame numbers for different modules. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([1, 2]
	...
	[Eiger] Only gets with result 2. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	[Eiger] Enable or disable show overflow flag in 32 bit mode. Default is disabled. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Package version. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	...
	[Gotthard2] Default: Parallel. Non parallel mode works only in continuous mode. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Sets detector measurement parameters to those contained in fname. Set up per measurement. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (det_id != -1) { throw RuntimeError("Cannot execute parameters at module level"); } 
...
os << R"V0G0N([0, 1]
	[Eiger] Sets up detector to do partial or complete reset at start of acquisition. 0 complete reset, 1 partial reset. Default is complete reset. Advanced function! )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	[Ctb][Mythen3][Xilinx Ctb] Gets the pattern file name including path of the last pattern uploaded. Returns an empty if nothing was uploaded or via a server default file )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([64 bit mask]
	[Ctb] 64 bit mask defining input (0) and output (1) signals. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([start addr] [stop addr] 
	[Ctb][Mythen3][Xilinx Ctb] Limits of complete pattern )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([64 bit mask]
	[Ctb][Mythen3][Xilinx Ctb] Selects the bits that will have a pattern mask applied to the selected patmask for every pattern. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([64 bit mask]
	[Ctb][Mythen3][Xilinx Ctb] Sets the mask applied to every pattern to the selected bits. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([fname]
	[Mythen3][Ctb][Xilinx Ctb] Loads ASCII pattern file directly to server (instead of executing line by line) )V0G0N" << std::endl;
return os.str();
}
...
pedestalmode [0]
		...
		Disabling pedestal mode will set back the normal mode values of #frames and #triggers. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (args[0] != "0") { throw RuntimeError("Unknown argument " + args[0] + ". Did you mean 0 to disable pedestal mode?"); }
det->setPedestalMode(defs::pedestalParameters());
os << ToString(args) << '\n';
}

if (args.size() == 2) {
det->setPedestalMode(defs::pedestalParameters(StringTo<uint8_t>(args[0]), StringTo<uint16_t>(args[1])));
os << ToString(args) << '\n';
}
...
os << R"V0G0N([duration] [(optional unit) ns|us|ms|s]
	Period between frames )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([pos|neg]
	[Mythen3] Sets negative or positive polarity. Default is positive )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n]
	Port number of the control server on detector for detector-client tcp interface. Default is 1952. Normally unchanged. Set different ports for virtual servers on same pc. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	...
	[Xilinx Ctb] Default is 0. Also configures the chip if powered on. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([name] 
		[Ctb][Xilinx_Ctb] Get the power index for the given name. )V0G0N" << std::endl;
return os.str();
}
...
defs::dacIndex index = defs::V_POWER_A;
if (det->getDetectorType().squash(defs::GENERIC) != defs::CHIPTESTBOARD && det->getDetectorType().squash(defs::GENERIC) != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute powerindex at module level"); } 
...
os << R"V0G0N([powername1 powername2 .. powername4] 
		[Ctb][Xilinx_Ctb] Set the list of power names for this board. )V0G0N" << std::endl;
return os.str();
}
...
auto det_type = det->getDetectorType().squash(defs::GENERIC);;
if (cmd != "daclist" && det_type != defs::CHIPTESTBOARD && det_type != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute powerlist at module level"); } 
...
os << R"V0G0N([0-4][name] 
		[Ctb][Xilinx_Ctb] Set the power at the given position to the given name. )V0G0N" << std::endl;
return os.str();
}
...
defs::dacIndex index = defs::V_POWER_A;
if (det->getDetectorType().squash(defs::GENERIC) != defs::CHIPTESTBOARD && det->getDetectorType().squash(defs::GENERIC) != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute powername at module level"); } 
auto t = det->getPowerName(static_cast<defs::dacIndex>(StringTo<int>(args[0]) + index));
os << args[0]<< ' '<< t << '\n';
}
...
if (det->getDetectorType().squash(defs::GENERIC) != defs::CHIPTESTBOARD && det->getDetectorType().squash(defs::GENERIC) != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute powername at module level"); } 
det->setPowerName(static_cast<defs::dacIndex>(StringTo<int>(args[0]) + index), args[1]);
os << ToString(args) << '\n';
}
...
os << R"V0G0N([name] 
		[Ctb][Xilinx_Ctb] Get values of all powers. )V0G0N" << std::endl;
return os.str();
}
...
if (t.size() > 0) {
                    ...
                    auto it = t.cbegin();                                              
                    os << ToString(*name_it++) << ' ';                                 
                    os << OutString(det->getPower(*it++, std::vector<int>{det_id}))<< suffix;                                                      
                    while (it != t.cend()) {                                         
                        os << ", " << ToString(*name_it++) << ' ';                     
                        os << OutString(det->getPower(*it++, std::vector<int>{det_id}))<< suffix;
                    }                                                                  
...
os << R"V0G0N([fname.pof | fname.rbf (full path)][(opitonal)--force-delete-normal-file]
	...
	[Mythen3][Gotthard2] Programs FPGA from rbf file (full path). Then, detector controller is rebooted. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 2) {
if (args[1] != "--force-delete-normal-file") { throw RuntimeError("Could not scan second argument. Did you mean --force-delete-normal-file?"); }
auto arg1 = StringTo<bool>("1");
det->programFPGA(args[0], arg1, std::vector<int>{ det_id });
os << "successful" << '\n';
}
...
os << R"V0G0N([n_times] [x] [y]
	[Eiger] Pulse pixel n number of times at coordinates (x, y). Advanced User! )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_times] 
	[Eiger] Pulse chip n times. If n is -1, resets to normal mode (reset chip completely at start of acquisition, where partialreset = 0). Advanced User! )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_times] [x] [y]
	[Eiger] Pulse pixel n number of times and moves relatively by (x, y). Advanced User! )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	[Mythen3] Enables or disables pump probe mode. Default is disabled. Pump probe mode only enables vth2. Disabling sets back to previous value. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	[Eiger] Sets detector size to a quad. 0 (disabled) is default. (Specific hardware required). )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (det_id != -1) { throw RuntimeError("Cannot execute quad at module level"); } 
auto arg0 = StringTo<bool>(args[0]);
det->setQuad(arg0);
os << args.front() << '\n';
}
...
os << R"V0G0N(
	...
	[8-512 (multiple of 8)]
		...
		[Moench] Number of rows per module starting from the centre. Options:16 - 400, must be multiples of 16. Default is 400. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	[Mythen3] Starts detector readout. Status changes to TRANSMITTING and automatically returns to idle at the end of readout. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 0) {
if (det_id != -1) { throw RuntimeError("Cannot execute readout at module level"); } 
...
os << R"V0G0N(
	...
	[Jungfrau] Default speed is half_speed. full_speed option only available from v2.0 boards and is recommended to set number of interfaces to 2. Also overwrites adcphase to recommended default.
	 [144|108]
		[Gotthard2] Readout speed of chip in MHz. Default is 108. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 0) {
auto det_type = det->getDetectorType().squash(defs::GENERIC);;
if (det_type == defs::CHIPTESTBOARD || det_type == defs::XILINX_CHIPTESTBOARD) { throw RuntimeError("ReadoutSpeed not implemented. Did you mean runclk?"); }
...
os << R"V0G0N(
	List of readout speed levels implemented for this detector. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	[Jungfrau][Moench][Ctb][Gotthard][Mythen3][Gotthard2][Xilinx Ctb] Reboot controller of detector. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([address] [32 bit value]
	...
	[Eiger] +0x100 for only left, +0x200 for only right. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([(optional) hard] 
	[Eiger][Jungfrau][Moench][Gotthard][Gotthard2][Mythen3]Reset dac values to the defaults. A 'hard' optional reset will reset the dacs to the hardcoded defaults in on-board detector server. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (args[0] != "hard") { throw RuntimeError("Unknown argument " + args[0] + ". Did you mean hard?"); }
...
os << R"V0G0N(
	[Jungfrau][Moench][Ctb][Xilinx Ctb] Reset FPGA. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([xmin] [xmax] 
	...
	Either all channels or a single adc or 2 chips (256 channels). Default is all channels enabled (-1 -1).  )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 2) {
defs::ROI t = defs::ROI(StringTo<int>(args[0]), StringTo<int>(args[1]));
if (det_id == -1 && det->size() > 1) { throw RuntimeError("Cannot execute ROI at multi module level"); }
det->setROI(t, det_id);
os << t << '\n';
}
...
os << R"V0G0N([analog|digital|analog_digital|transceiver|digital_transceiver]
	[Ctb][xilinx Ctb] Readout mode. [Ctb] Default is analog. [Xilinx Ctb] Default is Transceiver (only one implemented so far) )V0G0N" << std::endl;
return os.str();
}
...
StringTo<defs::readoutMode>(args[0]);
  ...
  throw RuntimeError("Could not convert argument 0 to defs::readoutMode");
}
}
...
os << R"V0G0N([n_clk in MHz]
	[Ctb] Run clock in MHz. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	Starts a thread in slsReceiver to arping the interface it is listening to every minute. Useful in 10G mode. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(Resets Region of interest in receiver. Default is all channels/pixels enabled. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 0) {
if (det_id != -1) { throw RuntimeError("Cannot execute rx_clearroi at module level"); } 
det->clearRxROI();
os << "successful" << '\n';
}
...
os << R"V0G0N([n_bytes]
	[Ctb] Offset in bytes in digital data to skip in receiver. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([nodiscard (default)|discardempty|discardpartial(fastest)]
	Frame discard policy of receiver. nodiscard does not discard frames, discardempty discards empty frames, discardpartial discards partial frames. )V0G0N" << std::endl;
return os.str();
}
...
StringTo<defs::frameDiscardPolicy>(args[0]);
  ...
  throw RuntimeError("Could not convert argument 0 to defs::frameDiscardPolicy");
}
}
...
os << R"V0G0N([n_frames]
	Set the number of frames in the receiver fifo depth (buffer between listener and writer threads). )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Current frame index received for each port in receiver during acquisition. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_frames]
	Number of frames per file in receiver in an acquisition. Default depends on detector type. 0 is infinite or all frames in single file. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([key1] [value1]
	[Receiver] Additional json header parameter streamed out from receiver. If not found in header, the pair is appended. An empty values deletes parameter. Max 20 characters for each key/value. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Client IP Address that last communicated with the receiver. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	Lock receiver to one client IP, 1 locks, 0 unlocks. Default is unlocked. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Number of missing packets for receiver. If negative, they are packets in excess. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	Partial frames padding enable in the receiver. Default: enabled. Disabling is fastest. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Actual udp socket buffer size. Double the size of rx_udpsocksize due to kernel bookkeeping. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	Switch on or off receiver text output during acquisition. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Starts receiver listener for detector data packets and create a data file (if file write enabled). )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 0) {
if (det_id != -1) { throw RuntimeError("Cannot execute rx_start at module level"); } 
det->startReceiver();
os << "successful" << '\n';
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (true) { throw RuntimeError("Cannot put. Did you mean to use command: \"rx_start\" or \"rx_stop\"?"); }
}
...
os << R"V0G0N(
	Stops receiver listener for detector data packets and closes current data file (if file write enabled). )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 0) {
if (det_id != -1) { throw RuntimeError("Cannot execute rx_stop at module level"); } 
det->stopReceiver();
os << "successful" << '\n';
}
...
os << R"V0G0N([port]
	TCP port for client-receiver communication. Default is 1954. Must be different if multiple receivers on same pc. Must be first command to set a receiver parameter. Multi command will automatically increment for individual modules. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Get kernel thread ids from the receiver in order of [parent, tcp, listener 0, processor 0, streamer 0, listener 1, processor 1, streamer 1, arping]. If no streamer yet or there is no second interface, it gives 0 in its place. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_size]
	UDP socket buffer size in receiver. Tune rmem_default and rmem_max accordingly. Max value is INT_MAX/2. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([nth frame]
	...
	If 0, streaming timer is the timeout, after which current frame is sent out. (default timeout is 500 ms). Usually used for gui purposes. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (det_id != -1) { throw RuntimeError("Cannot execute rx_zmqhwm at module level"); } 
auto arg0 = StringTo<int>(args[0]);
...
os << R"V0G0N([x.x.x.x]
	Zmq Ip Address from which data is to be streamed out of the receiver. Also restarts receiver zmq streaming if enabled. Default is from rx_hostname. Modified only when using an intermediate process between receiver. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([fnum]
	The starting frame index to stream out. 0 by default, which streams the first frame in an acquisition, and then depending on the rx zmq frequency/ timer )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	...
	Switching back to command line acquire will require disabling data streaming in receiver for fast applications.  )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (det_id != -1) { throw RuntimeError("Cannot execute savepattern at module level"); } 
...
os << R"V0G0N([dac_name|0|trimbits] [start_val] [stop_val] [step_size] [dac settling time ns|us|ms|s]
	...
	[Eiger][Mythen3] Use trimbits as dac name for a trimbit scan. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (StringTo<int>(args[0]) != 0) { throw RuntimeError("Unknown argument " + args[0] + ". Did you mean 0 to disable scan?"); }
if (det_id != -1) { throw RuntimeError("Cannot execute scan at module level"); } 
...
if (args.size() == 4) {
if (det_id != -1) { throw RuntimeError("Cannot execute scan at module level"); } 
det->setScan(defs::scanParameters(StringTo<defs::dacIndex>(args[0]), StringTo<int>(args[1]), StringTo<int>(args[2]), StringTo<int>(args[3])));
os << ToString(args) << '\n';
}
...
auto t = StringTo < time::ns > (tmp_time, unit);
if (det_id != -1) { throw RuntimeError("Cannot execute scan at module level"); } 
det->setScan(defs::scanParameters(StringTo<defs::dacIndex>(args[0]), StringTo<int>(args[1]), StringTo<int>(args[2]), StringTo<int>(args[3]), t));
os << ToString(args) << '\n';
}
...
os << R"V0G0N(
	Gets Scan error message if scan ended in error for non blocking acquisitions. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	[Jungfrau][Moench] The udp interface to stream data from detector. Effective only when number of interfaces is 1. Default: 0 (outer) )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([standard, fast, highgain, dynamicgain, lowgain, mediumgain, veryhighgain, highgain0, fixgain1, fixgain2, forceswitchg1, forceswitchg2, verylowgain, g1_hg, g1_lg, g2_hc_hg, g2_hc_lg, g2_lc_hg, g2_lc_lg, g4_hg, g4_lg, gain0]
	...
	[Moench] - [g1_hg | g1_lg | g2_hc_hg | g2_hc_lg | g2_lc_hg | g2_lc_lg | g4_hg | g4_lg] )V0G0N" << std::endl;
return os.str();
}
...
StringTo<defs::detectorSettings>(args[0]);
  ...
  throw RuntimeError("Could not convert argument 0 to defs::detectorSettings");
}
}
...
os << R"V0G0N([path]
	[Eiger][Mythen3] Directory where settings files are loaded from/to. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([name] 
		[Ctb][Xilinx_Ctb] Get the signal index for the given name. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 1) {
if (det->getDetectorType().squash(defs::GENERIC) != defs::CHIPTESTBOARD && det->getDetectorType().squash(defs::GENERIC) != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute signalindex at module level"); } 
...
os << R"V0G0N([signalname1 signalname2 .. signalname63] 
		[Ctb][Xilinx_Ctb] Set the list of signal names for this board. )V0G0N" << std::endl;
return os.str();
}
...
auto det_type = det->getDetectorType().squash(defs::GENERIC);;
if (cmd != "daclist" && det_type != defs::CHIPTESTBOARD && det_type != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute signallist at module level"); } 
...
os << R"V0G0N([0-63][name] 
		[Ctb][Xilinx_Ctb] Set the signal at the given position to the given name. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 1) {
if (det->getDetectorType().squash(defs::GENERIC) != defs::CHIPTESTBOARD && det->getDetectorType().squash(defs::GENERIC) != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute signalname at module level"); } 
auto arg0 = StringTo<int>(args[0]);
...
os << R"V0G0N([name] 
		[Ctb][Xilinx_Ctb] Get the slowadc index for the given name. )V0G0N" << std::endl;
return os.str();
}
...
defs::dacIndex index = defs::SLOW_ADC0;
if (det->getDetectorType().squash(defs::GENERIC) != defs::CHIPTESTBOARD && det->getDetectorType().squash(defs::GENERIC) != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute slowadcindex at module level"); } 
...
os << R"V0G0N([slowadcname1 slowadcname2 .. slowadcname7] 
		[Ctb][Xilinx_Ctb] Set the list of slowadc names for this board. )V0G0N" << std::endl;
return os.str();
}
...
auto det_type = det->getDetectorType().squash(defs::GENERIC);;
if (cmd != "daclist" && det_type != defs::CHIPTESTBOARD && det_type != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute slowadclist at module level"); } 
...
os << R"V0G0N([0-7][name] 
		[Ctb][Xilinx_Ctb] Set the slowadc at the given position to the given name. )V0G0N" << std::endl;
return os.str();
}
...
defs::dacIndex index = defs::SLOW_ADC0;
if (det->getDetectorType().squash(defs::GENERIC) != defs::CHIPTESTBOARD && det->getDetectorType().squash(defs::GENERIC) != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute slowadcname at module level"); } 
auto t = det->getSlowADCName(static_cast<defs::dacIndex>(StringTo<int>(args[0]) + index));
os << args[0]<< ' '<< t << '\n';
}
...
if (det->getDetectorType().squash(defs::GENERIC) != defs::CHIPTESTBOARD && det->getDetectorType().squash(defs::GENERIC) != defs::XILINX_CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute slowadcname at module level"); } 
det->setSlowADCName(static_cast<defs::dacIndex>(StringTo<int>(args[0]) + index), args[1]);
os << ToString(args) << '\n';
}
...
os << R"V0G0N([name] 
		[Ctb][Xilinx_Ctb] Get values of all slow adcs. )V0G0N" << std::endl;
return os.str();
}
...
if (t.size() > 0) {
                    ...
                    auto it = t.cbegin();                                              
                    os << ToString(*name_it++) << ' ';                                 
                    os << OutString(det->getSlowADC(*it++, std::vector<int>{det_id}))<< suffix;                                                      
                    while (it != t.cend()) {                                         
                        os << ", " << ToString(*name_it++) << ' ';                     
                        os << OutString(det->getSlowADC(*it++, std::vector<int>{det_id}))<< suffix;
                    }                                                                  
...
os << R"V0G0N(
	Starts detector acquisition. Status changes to RUNNING or WAITING and automatically returns to idle at the end of acquisition. If the acquisition was abruptly stopped, some detectors come back to STOPPED. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (true) { throw RuntimeError("Cannot put. Did you mean to use command: \"start\" or \"stop\"?"); }
}
...
os << R"V0G0N(
	Abort detector acquisition. Status changes to IDLE or STOPPED. Goes to stop server. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n]
	Port number of the stop server on detector for detector-client tcp interface. Default is 1953. Normally unchanged. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([duration (0-1638375 ns)] [(optional unit) ns|us|ms|s]
	[Jungfrau] Additional time delay between 2 consecutive exposures in burst mode (resolution of 25ns). Only applicable for chipv1.0. For advanced users only. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0-max]
	[Jungfrau] Storage cell that stores the first acquisition of the series. max is 15 (default) for chipv1.0 and 3 (default) for chipv1.1. For advanced users only. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([duration] [(optional unit) ns|us|ms|s]
	[Eiger] Dead time of EIGER subframes in 32 bit mode. Subperiod = subexptime + subdeadtime. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([duration] [(optional unit) ns|us|ms|s]
	[Eiger] Exposure time of EIGER subframes in 32 bit mode. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0, 1]
	[Jungfrau][Moench] Enables or disables synchronization between modules. Sync mode requires at least one master configured. Also requires flatband cabling between master and slave with termination board. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (det_id != -1) { throw RuntimeError("Cannot execute sync at module level"); } 
auto arg0 = StringTo<bool>(args[0]);
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getTemperature(defs::TEMPERATURE_10GE, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getTemperature(defs::TEMPERATURE_ADC, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getTemperature(defs::TEMPERATURE_DCDC, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
os << R"V0G0N([0]
	...
	If temperature crosses threshold temperature and temperature control is enabled, power to chip will be switched off and temperature event occurs. To power on chip again, temperature has to be less than threshold temperature and temperature event has to be cleared. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (StringTo<int>(args[0]) != 0) { throw RuntimeError("Unknown argument for temp event: ( " + args[0] + " ). Did you mean 0 to reset event?"); }
...
os << R"V0G0N([n_value]
	[Eiger][Jungfrau][Moench][Gotthard][Mythen3][Gotthard2][Xilinx CTB] FPGA Temperature )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getTemperature(defs::TEMPERATURE_FPGA, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getTemperature(defs::TEMPERATURE_FPGAEXT, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
os << R"V0G0N([n_value]
	[Eiger]Temperature of the left front end board fpga. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getTemperature(defs::TEMPERATURE_FPGA2, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
os << R"V0G0N([n_value]
	[Eiger]Temperature of the right front end board fpga. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getTemperature(defs::TEMPERATURE_FPGA3, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getTemperature(defs::SLOW_ADC_TEMP, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
os << R"V0G0N([n_value]
	[Eiger]Temperature close to the left so-dimm memory )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getTemperature(defs::TEMPERATURE_SODL, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
os << R"V0G0N([n_value]
	[Eiger]Temperature close to the right so-dimm memory )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getTemperature(defs::TEMPERATURE_SODR, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
os << R"V0G0N(
	List of temperature commands implemented for this detector. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Gets the values for every temperature for this detector. )V0G0N" << std::endl;
return os.str();
}
...
if (t.size() > 0) {
                    ...
                    auto it = t.cbegin();                                              
                    os << ToString(*it) << ' ';                                 
                    os << OutString(det->getTemperature(*it++, std::vector<int>{det_id}))<< suffix;                                                      
                    while (it != t.cend()) {                                         
                        os << ", " << ToString(*it) << ' ';                     
                        os << OutString(det->getTemperature(*it++, std::vector<int>{det_id}))<< suffix;
                    }                                                                  
...
os << R"V0G0N([0, 1]
	[Eiger][Ctb][Mythen3] 10GbE Enable. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([auto|trigger|gating|burst_trigger]
	...
	[Eiger] [auto|trigger|gating|burst_trigger] )V0G0N" << std::endl;
return os.str();
}
...
StringTo<defs::timingMode>(args[0]);
  ...
  throw RuntimeError("Could not convert argument 0 to defs::timingMode");
}
}
...
os << R"V0G0N([internal|external]
	[Gotthard2] Timing source. Internal is crystal and external is system timing. Default is internal. )V0G0N" << std::endl;
return os.str();
}
...
StringTo<defs::timingSourceType>(args[0]);
  ...
  throw RuntimeError("Could not convert argument 0 to defs::timingSourceType");
}
}
...
os << R"V0G0N([0, 1]
	[Eiger] Sets half module to top (1), else bottom. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([bitmask]
	[Ctb][Xilinx Ctb] Transceiver Enable Mask. Enable for each 4 Transceiver channel. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	[Eiger][Mythen3][Jungfrau][Moench] Sends software trigger signal to detector )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 0) {
bool block = false;
if (det_id != -1) { throw RuntimeError("Cannot execute trigger at module level"); } 
det->sendSoftwareTrigger(block);
os << "successful" << '\n';
}
...
os << R"V0G0N([n_triggers]
	Number of triggers per aquire. Set timing mode to use triggers. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 1) {
if (det_id != -1) { throw RuntimeError("Cannot execute triggers at module level"); } 
auto arg0 = StringTo<int64_t>(args[0]);
...
os << R"V0G0N(
	[Gotthard][Jungfrau][Moench][Mythen3][Gotthard2][Ctb][Xilinx Ctb] Number of triggers left in acquisition. Only when external trigger used. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([fname]
	[Eiger][Mythen3] Put will load the trimbit file to detector. If no extension specified, serial number of each module is attached. Get will save the trimbits from the detector to file with serial number added to file name. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_trimval]
	[Eiger][Mythen3] All trimbits set to this value. Returns -1 if all trimbits are different values. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value]
	[Ctb][Xilinx Ctb] Number of transceiver samples expected. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
if (det_id != -1) { throw RuntimeError("Cannot execute txdelay at module level"); } 
...
os << R"V0G0N([n_delay]
	...
	[Mythen3] [0-16777215] Each value represents 8 ns (125 MHz clock), max is 134 ms. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_delay]
	[Eiger] Transmission delay of first packet in an image being streamed out of the module's left UDP port. Each value represents 10ns. Typical value is 50000. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_delay]
	[Eiger] Transmission delay of first packet in an image being streamed out of the module's right UDP port. Each value represents 10ns. Typical value is 50000. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Returns detector type. Can be Eiger, Jungfrau, Gotthard, Moench, Mythen3, Gotthard2, ChipTestBoard, Xilinx_ChipTestBoard )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([ip=x.x.x.x] [(optional)ip2=x.x.x.x] 
		...
		The order of ip, mac and port does not matter. entry_value can be >0 only for [Eiger][Jungfrau][Moench][Mythen3][Gotthard2] where round robin is implemented. If 'auto' used, then ip is set to ip of rx_hostname. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
if (det_id == -1) { throw RuntimeError("Can execute udp_dstlist only at module level."); }
if (rx_id < 0 || rx_id >= MAX_UDP_DESTINATION) { throw RuntimeError("Invalid receiver index " + std::to_string(rx_id) + " to set round robin entry."); }
...
if (det_id == -1) { throw RuntimeError("Can execute udp_dstlist only at module level."); }
if (rx_id < 0 || rx_id >= MAX_UDP_DESTINATION) { throw RuntimeError("Invalid receiver index " + std::to_string(rx_id) + " to set round robin entry."); }
if (args.empty()) { throw RuntimeError("udp_dstlist require at least one argument."); }
...
os << R"V0G0N([x:x:x:x:x:x]
	Mac address of the receiver (destination) udp interface. Not mandatory to set as udp_dstip retrieves it from slsReceiver process, but must be set if you use a custom receiver (not slsReceiver). Use router mac if router between detector and receiver. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([x:x:x:x:x:x]
	[Jungfrau][Moench] Mac address of the receiver (destination) udp interface 2. Not mandatory to set as udp_dstip2 retrieves it from slsReceiver process but must be set if you use a custom receiver (not slsReceiver). 
	 ...
	 [Gotthard2] veto debugging. Use router mac if router between detector and receiver. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n]
	...
	If multi command, ports for each module is calculated (incremented by 1 if no 2nd interface) )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n]
	...
	[Jungfrau][Moench] top half or inner interface 
	[Eiger] right half 
	[Gotthard2] veto debugging )V0G0N" << std::endl;
return os.str();
}
...
[0-63]
	[Mythen3]
	 ...
	 One can set which is the first destination that the detector will stream images out from in a round robin fashion. The entry must not have been empty. Default: 0 )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	[Jungfrau][Moench][Eiger][Mythen3][Gotthard2] One can enter upto 32 (64 for Mythen3) destinations that the detector will stream images out in a round robin fashion. This is get only command. Default: 1 )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Reconfigures Detector with UDP destination. More for debugging as the configuration is done automatically when the detector has sufficient UDP details. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([x:x:x:x:x:x]
	...
	[Eiger] Do not set as detector will replace with its own DHCP Mac (1G) or DHCP Mac + 1 (10G). )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([x:x:x:x:x:x]
	[Jungfrau][Moench] Mac address of the top half or inner (source) udp interface.  )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	Validates that UDP configuration in the detector is valid. If not configured, it will throw with error message requesting missing udp information. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 2) {
if (args[args.size() - 1].find(".pof") == std::string::npos && args[args.size() - 1].find(".rbf") == std::string::npos) { throw RuntimeError("Programming file must be a pof/rbf file."); }
...
os << R"V0G0N([server_name  with full path]
	...
	[Jungfrau][Moench][Ctb]Also changes respawn server to the link, which is effective after a reboot. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([kernel_name with full path]
	...
	Updates the kernel image. Then, detector controller reboots with new kernel. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([0|1]
	Restart the detector server in update mode or not. This is useful when server-firmware compatibility is at its worst and server cannot start up normally )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N(
	User details from shared memory (hostname, type, PID, User, Date). )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
if (det_id != -1) { throw RuntimeError("Cannot execute user at module level"); } 
...
os << R"V0G0N([n_value]
	[Ctb][Xilinx Ctb] Power supply a in mV. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value]
	[Ctb][Xilinx Ctb] Power supply b in mV. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value]
	[Ctb][Xilinx Ctb] Power supply c in mV. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value]
	[Ctb] Power supply chip in mV. Do not use it unless you are completely sure you will not fry the board. )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getPower(defs::V_POWER_CHIP, std::vector<int>{ det_id });
os << OutString(t) << '\n';
}
...
os << R"V0G0N([n_value]
	[Ctb][Xilinx Ctb] Power supply d in mV. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value]
	[Ctb][Xilinx Ctb] Power supply io in mV. Minimum 1200 mV. Must be the first power regulator to be set after fpga reset (on-board detector server start up). )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value]
	[Ctb][Xilinx Ctb] Soft limit for power supplies (ctb only) and DACS in mV. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([chip index 0-9, -1 for all][10 bit hex value] 
	[Gotthard2] On chip Dac for comparator current of ADC. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 1) {
auto arg1 = StringTo<int>(args[0]);
auto t = det->getOnChipDAC(defs::VB_COMP_ADC, arg1, std::vector<int>{ det_id });
...
auto arg1 = StringTo<int>(args[0]);
auto arg2 = StringTo<int>(args[1]);
det->setOnChipDAC(defs::VB_COMP_ADC, arg1, arg2, std::vector<int>{ det_id });
os << args[0]<< ' '<< args[1] << '\n';
}
...
os << R"V0G0N([chip index 0-9, -1 for all][10 bit hex value] 
	[Gotthard2] On chip Dac for comparator current of analogue front end. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 1) {
auto arg1 = StringTo<int>(args[0]);
auto t = det->getOnChipDAC(defs::VB_COMP_FE, arg1, std::vector<int>{ det_id });
os << args[0]<< ' '<< OutStringHex(t) << '\n';
}
...
auto arg1 = StringTo<int>(args[0]);
auto arg2 = StringTo<int>(args[1]);
det->setOnChipDAC(defs::VB_COMP_FE, arg1, arg2, std::vector<int>{ det_id });
os << args[0]<< ' '<< args[1] << '\n';
}
...
os << R"V0G0N([chip index 0-9, -1 for all][10 bit hex value] 
	[Gotthard2] On chip Dac for current injection into preamplifier. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([chip index 0-9, -1 for all][10 bit hex value] 
	[Gotthard2] On chip Dac for opa current for driving the other DACs in chip. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 1) {
auto arg1 = StringTo<int>(args[0]);
auto t = det->getOnChipDAC(defs::VB_OPA_1ST, arg1, std::vector<int>{ det_id });
os << args[0]<< ' '<< OutStringHex(t) << '\n';
}
...
auto arg1 = StringTo<int>(args[0]);
auto arg2 = StringTo<int>(args[1]);
det->setOnChipDAC(defs::VB_OPA_1ST, arg1, arg2, std::vector<int>{ det_id });
os << args[0]<< ' '<< args[1] << '\n';
}
...
if (args.size() == 1) {
auto arg1 = StringTo<int>(args[0]);
auto t = det->getOnChipDAC(defs::VB_OPA_FD, arg1, std::vector<int>{ det_id });
os << args[0]<< ' '<< OutStringHex(t) << '\n';
}
...
auto arg1 = StringTo<int>(args[0]);
auto arg2 = StringTo<int>(args[1]);
det->setOnChipDAC(defs::VB_OPA_FD, arg1, arg2, std::vector<int>{ det_id });
os << args[0]<< ' '<< args[1] << '\n';
}
...
os << R"V0G0N([chip index 0-9, -1 for all][10 bit hex value] 
	[Gotthard2] On chip Dac for reference voltage of the comparator of analogue front end. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 1) {
auto arg1 = StringTo<int>(args[0]);
auto t = det->getOnChipDAC(defs::VREF_COMP_FE, arg1, std::vector<int>{ det_id });
os << args[0]<< ' '<< OutStringHex(t) << '\n';
}
...
auto arg1 = StringTo<int>(args[0]);
auto arg2 = StringTo<int>(args[1]);
det->setOnChipDAC(defs::VREF_COMP_FE, arg1, arg2, std::vector<int>{ det_id });
os << args[0]<< ' '<< args[1] << '\n';
}
...
os << R"V0G0N([0, 1]
	[Gotthard2] Enable or disable veto data data from chip. Default is 0. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([hits|raw] [lll|10gbe]
	[Gotthard2] Set the veto algorithm. Default is hits. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 1) {
defs::streamingInterface interface = StringTo<defs::streamingInterface>(args[0]);
}
...
defs::streamingInterface interface = StringTo<defs::streamingInterface>(args[1]);
}
...
if (args.size() == 1) {
defs::streamingInterface interface = StringTo<defs::streamingInterface>(args[0]);
if (interface == defs::streamingInterface::NONE) { throw RuntimeError("Must specify an interface to set algorithm"); }
...
os << R"V0G0N([chip index 0-9, -1 for all] [file name] 
	[Gotthard2] Set veto reference for each 128 channels for specific chip. The file should have 128 rows of gain index and 12 bit value in dec )V0G0N" << std::endl;
return os.str();
}
...
// generate code for each action
if (action == slsDetectorDefs::GET_ACTION) {
if (true) { throw RuntimeError("Cannot get vetofile. Did you mean vetophoton?"); }
}
...
[ichip] [output file]
	 Get gain indices and veto reference for 128 channels for chip ichip, saved to file. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([gain index] [12 bit value]
	[Gotthard2] Set veto reference for all 128 channels for all chips. )V0G0N" << std::endl;
return os.str();
}
...
// generate code for each action
if (action == slsDetectorDefs::GET_ACTION) {
if (true) { throw RuntimeError("Cannot get vetoref. Did you mean vetophoton?"); }
}
...
os << R"V0G0N([n_servers] [starting_port_number]
	Connecs to n virtual server at local host starting at specific control port. Every virtual server will have a stop port (control port + 1) )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::PUT_ACTION) {
if (args.size() == 2) {
if (det_id != -1) { throw RuntimeError("Cannot execute virtual at module level"); } 
auto arg0 = StringTo<int>(args[0]);
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredPower(defs::V_POWER_A, std::vector<int>{ det_id });
os << OutString(t) << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredPower(defs::V_POWER_B, std::vector<int>{ det_id });
os << OutString(t) << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredPower(defs::V_POWER_C, std::vector<int>{ det_id });
os << OutString(t) << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredPower(defs::V_POWER_D, std::vector<int>{ det_id });
os << OutString(t) << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredPower(defs::V_POWER_IO, std::vector<int>{ det_id });
os << OutString(t) << '\n';
}
...
os << R"V0G0N([n_limit] 
	...
	One must also set the receiver's send high water mark to similar value. Final effect is sum of them.
	 Setting it via command line is useful only before zmq enabled (before opening gui). )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([x.x.x.x]
	Ip Address to listen to zmq data streamed out from receiver or intermediate process. Default connects to receiver zmq Ip Address (from rx_hostname). Modified only when using an intermediate process between receiver and client(gui). Also restarts client zmq streaming if enabled. )V0G0N" << std::endl;
return os.str();
}
