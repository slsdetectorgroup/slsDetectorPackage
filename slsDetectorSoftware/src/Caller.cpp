      ...
      defs::dacIndex dacIndex =
          (det->getDetectorType().squash() == defs::CHIPTESTBOARD &&
           !is_int(args[0]))
              ? det->getDacIndex(args[0])
              : StringTo<defs::dacIndex>(args[0]);
      if (args.size() > 0 && action != defs::HELP_ACTION && is_int(args[0]) &&
          det->getDetectorType().squash() != defs::CHIPTESTBOARD) {
        throw RuntimeError("Dac indices can only be used for chip test board. "
...
pedestalmode [0]
  ...
  if (action == slsDetectorDefs::PUT_ACTION) {
    if (args.size() == 1) {
      if (args[0] != "0") {
        throw RuntimeError("Unknown argument " + args[0] + ". Did you mean 0?");
      }
  ...
  if (action == slsDetectorDefs::PUT_ACTION) {
    ...
    if (args.size() == 2) {
      if (args[1] != "--force-delete-normal-file") {
        throw RuntimeError("Could not scan second argument. Did you mean "
...
std::string Caller::temp_event(int action) {
  ...
  if (action == slsDetectorDefs::PUT_ACTION) {
    if (args.size() == 1) {
      if (StringTo<int>(args[0]) != 0) {
        throw RuntimeError("Unknown argument for temp event: ( " + args[0] +
...
std::string Caller::udp_dstlist(int action) {
  ...
  if (action == slsDetectorDefs::GET_ACTION) {
    if (args.size() == 0) {
      if (rx_id < 0 || rx_id >= MAX_UDP_DESTINATION) {
        throw RuntimeError("Invalid receiver index " + std::to_string(rx_id) +
  ...
  if (action == slsDetectorDefs::PUT_ACTION) {
    if (args.size() == 1) {
      if (rx_id < 0 || rx_id >= MAX_UDP_DESTINATION) {
        throw RuntimeError("Invalid receiver index to set round robin entry.");
      }
      if (det_id != -1) {
        throw RuntimeError("Cannot execute udp_dstlist at module level");
      }
...
std::string Caller::vm_a(int action) {
  ...
  if (action == slsDetectorDefs::GET_ACTION) {
    if (args.size() == 0) {
      auto t =
          det->getMeasuredPower(defs::V_POWER_A, std::vector<int>{ det_id });
      os << OutString(t) << " °C" << '\n';
    }
  }
...
std::string Caller::vm_b(int action) {
  ...
  if (action == slsDetectorDefs::GET_ACTION) {
    if (args.size() == 0) {
      auto t =
          det->getMeasuredPower(defs::V_POWER_B, std::vector<int>{ det_id });
      os << OutString(t) << " °C" << '\n';
    }
  }
...
std::string Caller::vm_c(int action) {
  ...
  if (action == slsDetectorDefs::GET_ACTION) {
    if (args.size() == 0) {
      auto t =
          det->getMeasuredPower(defs::V_POWER_C, std::vector<int>{ det_id });
      os << OutString(t) << " °C" << '\n';
    }
  }
...
std::string Caller::vm_d(int action) {
  ...
  if (action == slsDetectorDefs::GET_ACTION) {
    if (args.size() == 0) {
      auto t =
          det->getMeasuredPower(defs::V_POWER_D, std::vector<int>{ det_id });
      os << OutString(t) << " °C" << '\n';
    }
  }
...
std::string Caller::vm_io(int action) {
  ...
  if (action == slsDetectorDefs::GET_ACTION) {
    if (args.size() == 0) {
      auto t =
          det->getMeasuredPower(defs::V_POWER_IO, std::vector<int>{ det_id });
      os << OutString(t) << " °C" << '\n';
    }
  }
