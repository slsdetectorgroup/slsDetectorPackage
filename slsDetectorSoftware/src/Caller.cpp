...
os << '['<< ToStringHex(StringTo<int>(args[0]), 4)<< ", "<< OutStringHex(t, 16)<< "]" << '\n';
}
...
os << R"V0G0N([n_value]
	[Ctb] Voltage supply chip in mV. Do not use it unless you are completely sure you will not fry the board. )V0G0N" << std::endl;
return os.str();
}
...
os << R"V0G0N([n_value]
	[Ctb] Voltage supply io in mV. Minimum 1200 mV. Must be the first power regulator to be set after fpga reset (on-board detector server start up). )V0G0N" << std::endl;
return os.str();
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredVoltage(defs::V_POWER_A, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredVoltage(defs::V_POWER_B, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredVoltage(defs::V_POWER_C, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredVoltage(defs::V_POWER_D, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
if (action == slsDetectorDefs::GET_ACTION) {
if (args.size() == 0) {
auto t = det->getMeasuredVoltage(defs::V_POWER_IO, std::vector<int>{ det_id });
os << OutString(t)<< " °C" << '\n';
}
...
os << R"V0G0N([name] 
		[ChipTestBoard] Get the voltage index for the given name. )V0G0N" << std::endl;
return os.str();
}
...
defs::dacIndex index = defs::V_POWER_A;
if (det->getDetectorType().squash() != defs::CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute voltageindex at module level"); } 
...
os << R"V0G0N([voltagename1 voltagename2 .. voltagename4] 
		[ChipTestBoard] Set the list of voltage names for this board. )V0G0N" << std::endl;
return os.str();
}
...
if (args.size() == 0) {
if (cmd != "daclist" && det->getDetectorType().squash() != defs::CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute voltagelist at module level"); } 
...
os << R"V0G0N([0-4][name] 
		[ChipTestBoard] Set the voltage at the given position to the given name. )V0G0N" << std::endl;
return os.str();
}
...
defs::dacIndex index = defs::V_POWER_A;
if (det->getDetectorType().squash() != defs::CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute voltagename at module level"); } 
auto t = det->getVoltageName(static_cast<defs::dacIndex>(StringTo<int>(args[0]) + index));
os << args[0]<< ' '<< t << '\n';
}
...
if (det->getDetectorType().squash() != defs::CHIPTESTBOARD) { throw RuntimeError(cmd + " only allowed for CTB."); }
if (det_id != -1) { throw RuntimeError("Cannot execute voltagename at module level"); } 
det->setVoltageName(static_cast<defs::dacIndex>(StringTo<int>(args[0]) + index), args[1]);
os << ToString(args) << '\n';
}
...
os << R"V0G0N([name] 
		[ChipTestBoard] Get values of all voltages. )V0G0N" << std::endl;
return os.str();
}
...
if (t.size() > 0) {
                    ...
                    auto it = t.cbegin();                                              
                    os << ToString(*name_it++) << ' ';                                 
                    os << OutString(det->getVoltage(*it++, std::vector<int>{det_id}))<< suffix;                                                      
                    while (it != t.cend()) {                                         
                        os << ", " << ToString(*name_it++) << ' ';                     
                        os << OutString(det->getVoltage(*it++, std::vector<int>{det_id}))<< suffix;
                    }                                                                  
